                      ...
                      const IntVect& ngF, const IntVect& ngG, const bool aux_is_nodal)
{
    ...
    if (do_dive_cleaning)
    {
        F_fp[lev] = std::make_unique<MultiFab>(amrex::convert(ba, F_flag), dm, ncomps, ngF.max(), tag("F_fp"));
    }

    if (do_divb_cleaning)
    {
        G_fp[lev] = std::make_unique<MultiFab>(amrex::convert(ba, G_flag), dm, ncomps, ngG.max(), tag("G_fp"));
    }
